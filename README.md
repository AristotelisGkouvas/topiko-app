...
Web Push, χωρίς τρίτο μέρος: το μήνυμα κρυπτογραφείται εδώ με τα κλειδιά του
ίδιου του browser. Ένα γκολ ειδοποιεί **και τις δύο** ομάδες — είναι είδηση
για όποιον ακολουθεί οποιαδήποτε από τις δύο.

```bash
cd backend && ./.venv/Scripts/python.exe -m scripts.vapid_keys
```

Κενά κλειδιά = οι ειδοποιήσεις απλώς δεν προσφέρονται. Το κουμπί δεν
εμφανίζεται καν: prompt αδείας που ακολουθείται από σιωπή διαβάζεται ως
χαλασμένο, όχι ως αρύθμιστο.

Ειδοποιούν **μόνο** γκολ και τελικό σφύριγμα. Τηλέφωνο που ανάβει σε κάθε
κίτρινη κλείνει τις ειδοποιήσεις πριν το ημίχρονο.

> **Χρειάζεται HTTPS** στην παραγωγή (το `localhost` εξαιρείται). Χωρίς αυτό
> ο browser δεν επιτρέπει καν εγγραφή.

Η άδεια ζητείται **μόνο με πάτημα κουμπιού**. Prompt που εμφανίζεται μόνο του
απορρίπτεται από τους περισσότερους και δεν ξαναζητιέται ποτέ.
