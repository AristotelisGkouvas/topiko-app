...
Το `/diaxeirisi` ανοίγει στο **Φύλλο αγώνα**: επιλογή αγώνα της ημέρας και
μεγάλα κουμπιά — ένα γκολ είναι δύο πατήματα και καθόλου πληκτρολόγηση. Ο
σκόρερ συμπληρώνεται αργότερα, από κάποιον καθιστό· το να ζητάς όνομα τη
στιγμή του γκολ είναι ο τρόπος να μη γραφτεί ούτε το γκολ.

| Endpoint | |
|---|---|
| `GET /api/v1/{ένωση}/matches/{id}/feed` | Δημόσιο — η ροή ενός αγώνα |
| `GET /api/v1/{ένωση}/feed` | Δημόσιο — όλοι οι ζωντανοί μαζί |
| `POST /api/v1/{ένωση}/editor/matches/{id}/events` | Καταγραφή (`can_edit_live`) |
| `DELETE /api/v1/{ένωση}/editor/matches/{id}/events/{event_id}` | Αναίρεση |

**Τα γεγονότα είναι η εγγραφή· το σκορ είναι περίληψή τους.** Υπολογίζεται
ξανά μετά από κάθε καταχώρηση και κάθε αναίρεση, οπότε τα δύο δεν γίνεται να
αποκλίνουν και η «Αναίρεση» είναι μία διαγραφή αντί για διαγραφή συν αφαίρεση
που μπορεί να διαφωνήσει μαζί της. Αν αναιρεθεί όλη η ροή, το σκορ γίνεται
ξανά κενό — αλλιώς θα έμενε αποτέλεσμα που δεν το στηρίζει κανένα γεγονός.

Ένας αγώνας με ροή σημαδεύεται `manual_live`, ακριβώς όπως ένα πληκτρολογημένο
σκορ, ώστε οι κανόνες συμφιλίωσης να τον χειρίζονται ίδια και ο scraper να
υποχωρεί για το παράθυρό του.

> Το δικαίωμα είναι το υπάρχον `can_edit_live`. Ρόλος **γραμματέα ανά σύλλογο**
> θα ήταν στενότερος και είναι το προφανές επόμενο βήμα — θέλει πίνακα
> δικαιωμάτων με κλειδί τον σύλλογο, που το `user_associations` δεν είναι.

### Χωρίς σήμα

Τα μισά γήπεδα της Ηπείρου δεν έχουν κάλυψη πίσω από το τέρμα. Το φύλλο αγώνα
**δεν στέλνει απευθείας**: κάθε γεγονός γράφεται πρώτα σε ουρά στο κινητό και
φεύγει μόλις υπάρξει σύνδεση. Η οθόνη δείχνει «Χωρίς σήμα · 3 σε αναμονή».

Κάθε γεγονός παίρνει `client_id` **πριν** την πρώτη αποστολή, και ο server
αγνοεί ό,τι έχει ήδη καταγράψει. Χωρίς αυτό, ένα αίτημα που έφτασε αλλά του
οποίου η απάντηση χάθηκε θα ξαναστελνόταν και θα έβαζε το γκολ **δεύτερη
φορά** στον πίνακα.

Η ουρά φεύγει με τη σειρά και **σταματά στην πρώτη αποτυχία** αντί να την
προσπεράσει: είναι χρονολόγιο — γκολ, ημίχρονο, γκολ — και η εκτός σειράς
παράδοση θα έβαζε το σκορ ημιχρόνου σε λάθος σημείο.

Το service worker κρατά μόνο το κέλυφος του `/diaxeirisi`. Τα API responses
**δεν** μπαίνουν ποτέ σε cache — ένα μπαγιάτικο σκορ είναι λάθος σκορ.

## Ανακοινώσεις

Ο scraper αντιγράφει και τον πίνακα ανακοινώσεων (`/anakoinoseis`). Ένα
επιπλέον αίτημα ανά τρέξιμο, best effort: μια σπασμένη σελίδα ανακοινώσεων δεν
ρίχνει ένα τρέξιμο που η πραγματική του δουλειά είναι αγώνες και αποτελέσματα.

Τα ελληνικά στην πηγή είναι HTML entities — `&Eta;&Pi;&Sigma;` αντί για ΕΠΣ —
οπότε το parsing περνά από BeautifulSoup. Με regex θα αποθηκεύαμε τα entities
αυτούσια και το site θα τα έδειχνε στον αναγνώστη.

Δεν υπάρχει id ούτε ξεχωριστή διεύθυνση ανά ανακοίνωση, οπότε η ταυτότητα
είναι **τίτλος + ώρα δημοσίευσης**. Το σώμα ξαναγράφεται σε κάθε τρέξιμο: η
ένωση διορθώνει ανακοινώσεις επί τόπου, και μια διόρθωση που δεν φτάνει ως
εμάς είναι χειρότερη από καθόλου αντίγραφο.
